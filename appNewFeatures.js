...
// var sWins = 0;
// var playTo = 0;
// THIS BROKE EVERYTHING! KILL IT WITH FIRE!


//determine player based on turnCount
function player(){
   if((turnCount % 2) === 0) {
      //x is horseshoe
      return '<img src="images/horseshoe.png" id="X"></img>';
...
function makePlay(){
   ...
   if (this.getAttribute('playable')){
      ...
      this.innerHTML = player();
      this.setAttribute('playable', false);
      this.removeEventListener('click', makePlay);
   ...
   //after 5 turns check for a match:
   //earliest match can occur on 5th turn
   while (turnCount > 5) {
      checkForMatch();
   }
...
function checkForMatch(){
   ...
   //no match found
   return 'draw';
   console.log('done checking match');
};
...
function end(){
   ...
   for (var i =1; i<10; i++){
      var wipe = document.getElementById('id'+i);
      wipe.removeEventListener('click', makePlay);
   }
